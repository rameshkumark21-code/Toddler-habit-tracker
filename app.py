...
# Hide Streamlit's default header/footer branding elements for a clean look
hide_menu_style = """
    <style>
...
# 2. Raw HTML Tracker Code
# (Your complete updated interactive index.html code goes here)
html_code = """
<!DOCTYPE html>
<html lang="en">
<head>
...
<body style="margin: 0; padding: 0;">
  ...
  <script type="text/babel">
    ...
    function App() {
      ...
      const [childName, setChildName] = useState(() => localStorage.getItem("star_tracker_name") || "My Star!");
      ...
      const saveHabits = (newH) => { setHabits(newH); localStorage.setItem("star_tracker_habits", JSON.stringify(newH)); };
      const saveStars = (newS) => { setBoardStars(newS); localStorage.setItem("star_tracker_stars", JSON.stringify(newS)); };
      const saveName = (name) => { setChildName(name); localStorage.setItem("star_tracker_name", name); };
      const saveMilestones = (newM) => { setMilestones(newM); localStorage.setItem("star_tracker_milestones", JSON.stringify(newM)); };
      ...
      function submitStar() {
        ...
        saveStars([...boardStars, newStar]);
        if (targetHabit) {
          saveHabits(habits.map(hb => hb.id === targetHabit.id ? { ...hb, stars: hb.stars + 1 } : hb));
        }
        setPopup(null);
      }
          ...
          <div style={{ maxWidth: 560, margin: "0 auto", padding: "16px 14px 80px" }}>
            ...
            {page === "dashboard" && (
              <div>
                ...
                <div style={{ background: T.card, borderRadius: 20, padding: 16, boxShadow: T.shadow, marginBottom: 16 }}>
                  <div style={{ display: "flex", justify_content: "space-between", alignItems: "center", marginBottom: 4 }}>
                      ...
                      <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderBottom: i < milestones.length - 1 ? `1px solid ${T.navBorder}` : "none", opacity: done ? 1 : 0.65 }}>
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          <div style={{ fontSize: 12, color: T.subtext, display: "flex", alignItems: "center", gap: 4 }}>
                            <input className="milestone-input" type="number" value={m.stars} onChange={e => handleMilestoneChange(i, "stars", e.target.value)} style={{ width: 40, textAlign: "center", background: "rgba(0,0,0,0.03)", borderRadius: 4 }} /> stars required
                          </div>
                        </div>
...
# 3. Render inside Streamlit frame securely
