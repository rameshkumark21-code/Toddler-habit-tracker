...
# ─── 2. GOOGLE SHEETS CONNECTION (optional) ────────────────────────────────
...
def load_cloud_data():
    """Load data from Google Sheets. Falls back to defaults if unavailable."""
    if conn:
        try:
            ...
            # Ensure boolean types survive the round-trip
            for h in habits:
...
def save_cloud_data(habits, stars, child_name, theme_key):
    """Persist data back to Google Sheets."""
    if not conn:
        return
    try:
        # Habits sheet
        ...
        # Stars sheet — handle empty list safely
        if stars:
            stars_df = pd.DataFrame(stars)
        else:
            stars_df = pd.DataFrame(columns=["id", "x", "y", "habitName", "habitEmoji", "icon", "rot", "size"])
        conn.update(worksheet="Stars", data=stars_df)

        # Settings sheet
        settings_df = pd.DataFrame([{"child_name": child_name, "theme_key": theme_key}])
...
# We inject current state as JSON into the HTML on every render.
# The React app reads these as initial values, runs in the iframe,
# and posts messages back to Streamlit when state changes.
...
<body>
  ...
  <script type="text/babel">
      ...
      function playMilestoneFanfare() {{
        try {{
          ...
          melody.forEach((freq,i) => {{
            ...
            osc.start(ctx.currentTime+times[i]); osc.stop(ctx.currentTime+times[i]+0.4);
          }});
          const boom=ctx.createOscillator(), bGain=ctx.createGain();
          boom.connect(bGain); bGain.connect(ctx.destination); boom.type="sine";
          ...
          boom.start(ctx.currentTime); boom.stop(ctx.currentTime+0.55);
        }} catch(e) {{}}
      }}
    ...
    function ConfettiCanvas({{ colors, onDone }}) {{
      ...
      return React.createElement("canvas",{{ ref:canvasRef, style:{{ position:"fixed",inset:0,zIndex:9999,pointerEvents:"none",width:"100vw",height:"100vh" }} }});
    }}
    ...
    function MilestoneToast({{ milestone, onDone }}) {{
      ...
      return (
        <div style={{{{
          ...
          background:"linear-gradient(135deg,#FFD700,#FF8C00)",borderRadius:28,
          padding:"32px 40px",textAlign:"center",boxShadow:"0 16px 60px rgba(0,0,0,0.35)",
          zIndex:9998,animation:"toastIn 0.4s cubic-bezier(.34,1.56,.64,1)",minWidth:240,
        }}}}>
    ...
    function App() {{
      ...
      // ── Persist to Python/Streamlit via postMessage ──
      ...
      // Check milestones when stars change
      useEffect(() => {{
      ...
      function placeStarForHabit(habit) {{
        ...
        const newStar={{
          ...
          x:Math.max(4,Math.min(93,sx)), y:Math.max(4,Math.min(93,sy)),
        ...
        if(habit) newHabits=habits.map(hb=>hb.id===habit.id?{{...hb,stars:hb.stars+1}}:hb);
        ...
        <div style={{{{minHeight:"100vh",background:T.pageBg,fontFamily:"'Nunito',sans-serif",color:T.text}}}}>
          ...
          <nav style={{{{background:T.navBg,borderBottom:`2px solid ${{T.navBorder}}`,padding:"0 14px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:300,boxShadow:T.shadow}}}}>
            <div style={{{{display:"flex"}}}}>
              {{[{{key:"board",label:`${{T.habitIcons.navIcon}} Board`}},{{key:"dashboard",label:"📊 Dashboard"}},{{key:"manage",label:"⚙️ Habits"}}].map(tab=>(
                <button key={{{{tab.key}}}} className="nav-tab" onClick={{{{()=>setPage(tab.key)}}}}
                  style={{{{fontFamily:"'Nunito',sans-serif",color:page===tab.key?T.primary:T.subtext,borderBottom:page===tab.key?`3px solid ${{T.primary}}`:"3px solid transparent"}}}}>
                  {{tab.label}}
                </button>
              ))}}
            </div>
            <button onClick={{{{()=>setShowThemePanel(p=>!p)}}}} style={{{{background:T.primary,color:"#fff",border:"none",borderRadius:20,padding:"6px 14px",fontWeight:700,fontSize:13,cursor:"pointer"}}}}>🎨 Theme</button>
          </nav>
          ...
          {{showThemePanel && (
            <div style={{{{position:"fixed",top:54,right:12,background:T.card,borderRadius:16,padding:16,zIndex:400,boxShadow:"0 8px 32px rgba(0,0,0,0.22)",border:`2px solid ${{T.navBorder}}`,minWidth:190}}}}>
              ...
              {{Object.entries(THEMES).map(([key,th])=>(
                <button key={{{{key}}}} onClick={{{{()=>changeTheme(key)}}}} style={{{{display:"block",width:"100%",textAlign:"left",padding:"9px 14px",marginBottom:4,borderRadius:10,border:themeKey===key?`2px solid ${{T.primary}}`:"2px solid transparent",background:themeKey===key?T.primary+"20":"transparent",fontWeight:700,fontSize:14,cursor:"pointer",color:T.text}}}}>
                  {{th.name}}
                </button>
              ))}}
            </div>
          ...
          <div style={{{{maxWidth:560,margin:"0 auto",padding:"16px 14px 80px"}}}}>
            ...
            {{page==="board" && (
              <div>
                ...
                <div style={{{{position:"relative",width:"100%",paddingTop:"75%",background:T.boardBg,borderRadius:18,border:`8px solid ${{T.boardBorder}}`,boxShadow:T.boardShadow,cursor:"crosshair",overflow:"hidden",marginBottom:14}}}}
                  ...
                  {{T.habitIcons.boardDecor.map((d,i)=>{{
                    const positions=[{{top:"4%",left:"3%"}},{{top:"4%",right:"3%"}},{{bottom:"4%",left:"3%"}},{{bottom:"4%",right:"3%"}},{{top:"4%",left:"44%"}},{{bottom:"4%",left:"44%"}}];
                    return <div key={{{{i}}}} style={{{{position:"absolute",fontSize:22,...positions[i%6],opacity:0.55,zIndex:2,pointerEvents:"none",animation:`boardDecorFloat ${{2.5+i*0.4}}s ease-in-out infinite`}}}}>{{d}}</div>;
                  }})}}
                  ...
                  {{boardStars.map(star=>(
                    <div key={{{{star.id}}}} className="board-star" title={{{{star.habitName}}}}
                      style={{{{left:`${{star.x}}%`,top:`${{star.y}}%`,fontSize:star.size,"--rot":`${{star.rot}}deg`,"--glow":T.starGlow,"--glow2":T.starGlow,transform:`rotate(${{star.rot}}deg)`,zIndex:10,lineHeight:1}}}}>
                      {{star.icon}}
                      <div style={{{{position:"absolute",top:"100%",left:"50%",transform:`translateX(-50%) rotate(${{-star.rot}}deg)`,background:T.labelBg,color:T.labelColor,fontSize:8,fontWeight:800,whiteSpace:"nowrap",padding:"1px 4px",borderRadius:4,marginTop:2,boxShadow:"0 1px 4px rgba(0,0,0,0.15)",maxWidth:64,overflow:"hidden",textOverflow:"ellipsis"}}}}>
                  ...
                  {{popup && (()=>{{
                    ...
                    return (
                      <div onClick={{{{e=>e.stopPropagation()}}}} style={{{{position:"absolute",left,...(above?{{bottom:(rect?.height??400)-(popup.clientY-(rect?.top??0))+12}}:{{top:rawT}}),width:popW,background:T.card,borderRadius:16,padding:"14px 12px",boxShadow:"0 8px 32px rgba(0,0,0,0.28)",border:`2px solid ${{T.navBorder}}`,zIndex:50}}}}>
                        <div style={{{{fontWeight:800,fontSize:13,marginBottom:10,color:T.text}}}}>⭐ Which habit earned a star?</div>
                        <div style={{{{maxHeight:150,overflowY:"auto",marginBottom:8}}}}>
                          {{activeHabits.map(h=>(
                            <div key={{{{h.id}}}} className="habit-pill" style={{{{background:T.primary+"18",borderColor:T.navBorder}}}} onClick={{{{()=>placeStarForHabit(h)}}}}>
                        ...
                        <div style={{{{borderTop:`1px solid ${{T.navBorder}}`,paddingTop:8}}}}>
                          ...
                          <div style={{{{display:"flex",gap:6}}}}>
                            <input value={{{{customHabit}}}} onChange={{{{e=>setCustomHabit(e.target.value)}}}}
                              ...
                              style={{{{flex:1,padding:"7px 10px",borderRadius:10,border:`1.5px solid ${{T.navBorder}}`,fontSize:12,fontFamily:"'Nunito',sans-serif",background:T.navBg,color:T.text,outline:"none"}}}} />
                            <button onClick={{{{placeCustomStar}}}} style={{{{background:T.primary,color:"#fff",border:"none",borderRadius:10,padding:"7px 12px",fontWeight:800,fontSize:16,cursor:"pointer"}}}}>⭐</button>
                          </div>
                        </div>
                        <button onClick={{{{()=>setPopup(null)}}}} style={{{{marginTop:8,width:"100%",padding:"6px",background:"transparent",border:`1.5px solid ${{T.navBorder}}`,borderRadius:10,color:T.subtext,fontSize:12,fontWeight:700,cursor:"pointer"}}}}>Cancel</button>
                      </div>
                    );
                  }})()}}
                </div>
                <div style={{{{display:"flex",gap:8}}}}>
                  <button onClick={{{{resetBoard}}}} style={{{{flex:1,padding:"10px",border:`2px solid ${{T.primary}}`,background:"transparent",borderRadius:12,color:T.primary,fontWeight:800,fontSize:13,cursor:"pointer"}}}}>🔄 Clear Board</button>
                  <button onClick={{{{()=>setPage("dashboard")}}}} style={{{{flex:1,padding:"10px",background:`linear-gradient(135deg,${{T.primary}},${{T.secondary}})`,border:"none",borderRadius:12,color:"#fff",fontWeight:800,fontSize:13,cursor:"pointer"}}}}>📊 View Dashboard</button>
                </div>
              </div>
            )}}
              ...
              <div>
                ...
                <div style={{{{background:T.card,borderRadius:20,padding:16,boxShadow:T.shadow,marginBottom:16}}}}>
                  ...
                  {{MILESTONES.map((m,i)=>{{
                    ...
                    return (
                      <div key={{{{m.stars}}}} style={{{{display:"flex",alignItems:"center",gap:12,padding:"10px 0",borderBottom:i<MILESTONES.length-1?`1px solid ${{T.navBorder}}`:"none",opacity:done?1:0.5}}}}>
                        <div style={{{{width:36,height:36,borderRadius:"50%",background:done?colors[i]:T.navBorder,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}}}>{{done?"✅":"🔒"}}</div>
                        <div style={{{{flex:1}}}}>
                          <div style={{{{fontWeight:700,fontSize:14}}}}>{{m.reward}}</div>
                          <div style={{{{fontSize:12,color:T.subtext}}}}>{{m.stars}} stars needed</div>
                        </div>
                        {{done && <div style={{{{fontSize:11,fontWeight:800,color:colors[i]}}}}>EARNED!</div>}}
                      </div>
                    );
                  }}}}
                </div>
                <button onClick={{{{resetBoard}}}} style={{{{width:"100%",padding:"13px",border:`2px solid ${{T.primary}}`,background:"transparent",borderRadius:14,color:T.primary,fontWeight:800,fontSize:14,cursor:"pointer"}}}}>🔄 Reset All Stars</button>
              </div>
            )}}
            ...
            {{page==="manage" && (
              <div>
                ...
                <div style={{{{background:T.card,borderRadius:20,padding:16,boxShadow:T.shadow,marginBottom:16}}}}>
                  ...
                  <input value={{{{childName}}}} onChange={{{{e=>updateChildName(e.target.value)}}}} placeholder="Enter name..."
                    style={{{{width:"100%",padding:"10px 14px",borderRadius:12,border:`2px solid ${{T.navBorder}}`,fontFamily:"'Nunito',sans-serif",fontSize:15,fontWeight:700,color:T.text,background:T.navBg,outline:"none",boxSizing:"border-box"}}}} />
                </div>
                <div style={{{{background:T.card,borderRadius:20,padding:16,boxShadow:T.shadow,marginBottom:16}}}}>
                  ...
                  <div style={{{{display:"flex",gap:8,marginBottom:10}}}}>
                    <input value={{{{newHabitEmoji}}}} onChange={{{{e=>setNewHabitEmoji(e.target.value)}}}}
                      style={{{{width:56,padding:"9px",borderRadius:12,textAlign:"center",border:`2px solid ${{T.navBorder}}`,fontSize:20,background:T.navBg,color:T.text,outline:"none"}}}} />
                    <input value={{{{newHabitName}}}} onChange={{{{e=>setNewHabitName(e.target.value)}}}}} placeholder="Habit name..."
                      onKeyDown={{{{e=>e.key==="Enter"&&addHabit()}}}}
                      style={{{{flex:1,padding:"9px 12px",borderRadius:12,border:`2px solid ${{T.navBorder}}`,fontSize:14,fontFamily:"'Nunito',sans-serif",background:T.navBg,color:T.text,outline:"none"}}}} />
                  </div>
                  <button onClick={{{{addHabit}}}} style={{{{width:"100%",padding:"11px",background:`linear-gradient(135deg,${{T.primary}},${{T.secondary}})`,border:"none",borderRadius:12,color:"#fff",fontWeight:800,fontSize:14,cursor:"pointer"}}}}>Add Habit</button>
                </div>
                    ...
                    <div key={{{{h.id}}}} style={{{{borderBottom:`1px solid ${{T.navBorder}}`,paddingBottom:12,marginBottom:12}}}}>
                      {{editingId===h.id ? (
                        <div style={{{{display:"flex",gap:8,alignItems:"center"}}}}>
                          ...
                          <button onClick={{{{()=>saveEdit(h.id)}}}} style={{{{background:T.secondary,border:"none",borderRadius:8,padding:"7px 11px",color:"#fff",fontWeight:700,cursor:"pointer"}}}}>✓</button>
                        ...
                        <div style={{{{display:"flex",alignItems:"center",gap:10}}}}>
                          ...
                          <button onClick={{{{()=>toggleHabit(h.id)}}}} style={{{{padding:"4px 9px",borderRadius:20,border:"none",background:h.active?T.secondary+"33":T.navBorder,color:h.active?T.secondary:T.subtext,fontSize:11,fontWeight:800,cursor:"pointer"}}}}>{{h.active?"ON":"OFF"}}</button>
                          ...
                          <button onClick={{{{()=>deleteHabit(h.id)}}}} style={{{{width:30,height:30,borderRadius:8,border:"none",background:"#ffebee",cursor:"pointer",fontSize:14}}}}>🗑️</button>
                        </div>
                      )}}
                    </div>
                  ))}}
...
# FIX: Always rewrite the file so Python-side state changes are reflected.
# We use a content hash to avoid unnecessary disk I/O on identical content.
...
existing_hash = st.session_state.get("html_hash", "")

# Only write when content actually changed — prevents unnecessary I/O
...
if val is not None:
    ...
    # FIX: Only rerun when state genuinely changed — prevents infinite rerun loops
    if state_hash != st.session_state.get("last_hash", ""):
        ...
        # Persist to Google Sheets (non-blocking via try/except in save_cloud_data)
